...
퍼블릭 오픈 없이 **내부/소수 고객 대상으로 먼저 운영**한다. 실행 인프라 비용과 안정성이 확보된 이후에 퍼블릭 출시를 검토한다.
...
| 유형 | 특성 |
|---|---|
| 지인 / 개발자 그룹 | 직접 설명 가능, 불편함을 감수하며 피드백이 빠름 |
| 사전 연락된 특정 팀/회사 | 최소 설명으로 바로 사용 가능해야 함, admin이 초기 세팅 담당 |

### 이 전략에서 실질적으로 중요한 것

- **계정 생성은 admin이 수동으로** — self-serve 가입 불필요, admin UI에서 직접 생성으로 충분
- **모델/연결을 admin UI에서 미리 세팅** — 고객은 로그인 → 프로젝트 → 채팅 → 세션으로 바로 진입 가능
- **audit log / session log는 초기부터 필요** — 소수 고객이라도 장애/이슈 발생 시 원인 파악이 가능해야 함

### Phase 순서에 미치는 영향

- Phase 0~6은 기존 순서 유지
- Phase 3~4(세션/브라우저)는 개발 완료 후 소수 고객 대상으로만 운영, 비용 예측 가능한 범위(클러스터 1~2개)에서 시작
- self-serve onboarding, 결제 연동, 퍼블릭 확장성 최적화는 퍼블릭 출시 단계(Phase 9 이후)로 미룸
